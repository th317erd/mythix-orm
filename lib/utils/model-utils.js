...
async function getRelationalModelStatusForField(connection, self, field, ...args) {
  ...
  const addModelStatus = (relation, opposingRelation, status) => {
    let modelRelation = relationalMap[relation.modelName];
    if (!modelRelation) {
      let fields = new Map();

      if (opposingRelation) {
        // this key/value pair must be flipped unnaturally
        // otherwise the map might not hold all the fields
        // we need it to hold. With the target model being
        // the key, we ensure we can have any number of
        // target models and fields.
        fields.set(`${opposingRelation.modelName}:${opposingRelation.fieldName}`, `${relation.modelName}:${relation.fieldName}`);
      }

      relationalMap[relation.modelName] = Object.assign(status, { Model: relation.Model, modelName: relation.modelName }, { fields });
    } else if (status.instance) {
      let fields = modelRelation.fields || new Map();

      if (opposingRelation)
        fields.set(`${opposingRelation.modelName}:${opposingRelation.fieldName}`, `${relation.modelName}:${relation.fieldName}`);

      relationalMap[relation.modelName] = Object.assign(relationalMap[relation.modelName], status, { fields });
    }
  };
  ...
  const addRelationalStatus = async (source, target) => {
    if (field.type.isManyRelation()) {
      // we need to create
      addModelStatus(source, target, { create: true });
      addModelStatus(target, source, { create: true });
      return;
    }

    let model = findAvailableRelationalModel(source.modelName, source.field);
    if (!hasValidPrimaryKey(model))
      addModelStatus(source, target, { create: true, instance: model });
    else
      addModelStatus(source, target, { create: false, instance: model });

    model = findAvailableRelationalModel(target.modelName, target.field);
    if (!hasValidPrimaryKey(model))
      addModelStatus(target, source, { create: true, instance: model });
    else
      addModelStatus(target, source, { create: false, instance: model });
  };

  let query = await field.type.walkQueryRelations(connection, async ({ source, target }) => {
    await addRelationalStatus(source, target);
  }, { self, field: field }, ...args);

  // TODO: Move to walkQueryRelations
  let queryContext    = query._getRawQueryContext();
  ...
  let model = findAvailableRelationalModel(targetModelName, field);
  if (!hasValidPrimaryKey(model))
    addModelStatus(rootTarget, originSource, { create: true, instance: model });
  else
    addModelStatus(rootTarget, originSource, { create: false, instance: model });
...
async function createAndSaveAllRelatedModels(connection, self, field, allModelAttributes, options) {
  ...
  let TargetModel     = await field.type.getTargetModel(connection, { self, field });
  let targetModelName = TargetModel.getModelName();
