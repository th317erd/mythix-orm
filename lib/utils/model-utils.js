...
function injectModelMethod(modelInstance, method, methodName, fullMethodName) {
  ...
  if (!(methodName in modelInstance)) {
    Object.defineProperties(modelInstance, {
      [methodName]: {
        writable:     true,
        enumberable:  false,
...
function getRelationalModelStatusForField(connection, modelInstance, originField) {
  ...
  const addModelStatus = (Model, modelName, source, target, status) => {
    ...
    if (!target.fieldType.isRelational()) {
      let SourceModel     = source.fieldType.getTargetModel(connection, { recursive: true, followForeignKeys: true });
      let sourceField     = source.fieldType.getTargetField(connection);
    ...
    let TargetModel = target.fieldType.getTargetModel(connection, { recursive: false, followForeignKeys: false });
  ...
  originField.type.walkSourceRelation(connection, ({ source, target }) => {
...
function constructModelsForCreationFromOriginField(connection, modelInstance, originField, attributes) {
  ...
  let TargetModel       = originField.type.getTargetModel(connection, { recursive: true, followForeignKeys: true });
  let targetModelName   = TargetModel.getModelName();
  ...
  for (let i = 0, il = sortedModelNames.length; i < il; i++) {
    ...
    setRelationalValues(connection, TargetModel, thisModelInstance, modelInstance.getModel(), modelInstance);
...
function setRelationalValues(connection, Model, modelInstance, RelatedModel, relatedModelInstance) {
  let fieldsToSet       = {};
  ...
  Model.iterateFields(({ field, fieldName }) => {
    ...
    if (fieldType.isRelational()) {
      let sourceField = fieldType.getSourceField(connection, { recursive: true, followForeignKeys: true });
      if (sourceField.Model.getModelName() !== modelName)
        return;

      let targetField = fieldType.getTargetField(connection, { recursive: true, followForeignKeys: true });
