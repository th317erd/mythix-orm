...
function margeFields(queryEngine, currentFields, _incomingFields, extraData, _options) {
  ...
  for (let i = 0, il = incomingFields.length; i < il; i++) {
    ...
    if (typeof incomingField.isLiteral === 'function' && incomingField.isLiteral(incomingField)) {
      ...
      let fullyQualifiedFieldName = (typeof incomingField.getFullyQualifiedFieldName === 'function') ? incomingField.getFullyQualifiedFieldName() : incomingField.toString(connection, options);
      addOrRemove(mode, fullyQualifiedFieldName, incomingField);
      continue;
    }
