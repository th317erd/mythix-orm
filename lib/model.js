...
class Model {
  ...
  // Simple convenience method to
  // help collect results from queries
  static collect(asyncGenerator) {
    return Utils.collect(asyncGenerator);
  }
  ...
  static getQueryEngine() {
    ...
    let defaultScope  = this.defaultScope(queryEngine);

    if (QueryEngine.isQuery(defaultScope) && (defaultScope._isLastPartCondition() || !defaultScope._isLastPartControl()))
      defaultScope = defaultScope.AND[this.getModelName()];
  ...
  static iterateFields(callback, _fields) {
    ...
    for (let i = 0, il = fieldNames.length; i < il; i++) {
      ...
      if (!field.type)
        throw new Error(`${this.name}::iterateFields: "type" not found on "${this.getModelName()}.${fieldName}". "type" is required for all fields.`);
  ...
  hasValidPrimaryKey() {
    ...
    return (pkValue != null && pkField.type.isValidValue(pkValue));
  }
  ...
  async destroy(options) {
    ...
    return await this.getModel().where[primaryKeyFieldName].EQ(this.id).destroy(options);
  }
...
Utils.iterateStaticProps(Model, ({ value, key, prototype }) => {
  ...
  if (!(key in prototype)) {
    prototype[key] = function(...args) {
      return this.constructor[key].apply(this.constructor, args);
    };
  }
});
