...
const TYPE_OPERATIONS = {
  'queryFor': async function({ field, type }, ...args) {
    return await type.prepareQuery(null, this, field, ...args);
  },
  ...
  'update': async function({ fullMethodName, field, type, create }, attributes, _options) {
    let options = _options || {};

    let result = await this.getConnection(options.connection).transaction(async (connection) => {
      let query = await type.prepareQuery(connection, this, field, null, options);
  ...
  'exists': async function({ field, type }, options) {
    let query = await type.prepareQuery(null, this, field, null, options);
