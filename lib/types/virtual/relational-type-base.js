...
class RelationalTypeBase extends Type {
  ...
  _walkRelationFields(type, callback, _connection) {
    ...
    for (let i = 0, il = fieldNames.length; i < il; i++) {
      ...
      let result      = callback({ type, source: sourceArgs, target: targetArgs, stop });
      ...
      if ((i + 1) >= il) {
        if (fieldType.isForeignKey()) {
          ...
          result = callback({ type, source: targetArgs, target: fieldToArgs(targetField) });
  ...
  _getModelRelations(connection) {
    ...
    this._walkRelationFields('target', ({ source, target }) => {
      let sourceField = (target.fieldType.isRelational()) ? source.fieldType.getSourceField({ recursive: true, followForeignKeys: false }) : source.field;
      let targetField = (target.fieldType.isRelational()) ? source.fieldType.getTargetField({ recursive: true, followForeignKeys: false }) : target.field;
    ...
    this._walkRelationFields('source', ({ source, target }) => {
      let sourceField = (source.fieldType.isRelational()) ? source.fieldType.getSourceField({ recursive: true, followForeignKeys: false }) : source.field;
      let targetField = (source.fieldType.isRelational()) ? source.fieldType.getTargetField({ recursive: true, followForeignKeys: false }) : target.field;
