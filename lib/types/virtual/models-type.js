...
async function destroyRelatedModels(connection, field, type, TargetModel, storedModels, invertMatch, options, operation) {
  ...
  let { relationMap }     = await Utils.getRelationalModelStatusForField(connection, this, field);
...
const TYPE_OPERATIONS = {
  ...
  'get': function({ field, type }, _queryEngine, _options) {
    const doGet = async function*() {
      let query                       = await type.prepareQuery(null, this, field, queryEngine, options);
  ...
  'set': async function({ field, type, addTo }, models, options) {
    return this.getConnection(options && options.connection).transaction(async (connection) => {
      let TargetModel = await type.getTargetModel(connection, { self: this, field });
  ...
  'removeFrom': async function({ field, type, get }, _models, _options) {
    return this.getConnection(_options && _options.connection).transaction(async (connection) => {
      let { relationMap, TargetModel } = await Utils.getRelationalModelStatusForField(connection, this, field);
      ...
      if (await needsLoad()) {
        let query = Utils.buildQueryFromModelsAttributes(connection, TargetModel, models);
        if (!query)
        ...
        storedModels = await get.call(this, query, Object.assign({}, options, { stream: false, includeRelations: true, connection }));
      } else {
        storedModels = models;
      }
  ...
  'destroy': async function({ field, type }, queryEngine, options) {
    let query = await type.prepareQuery(null, this, field, queryEngine, options);
    return await query.destroy(options);
  },
  'count': async function({ field, type }, queryEngine, options) {
    let query = await type.prepareQuery(null, this, field, queryEngine, options);
    return await query.count(null, options);
  },
  'has': async function({ count }, queryEngine, options) {
    let itemCount = await count.call(this, queryEngine, options);
    return (itemCount > 0);
  },
};
