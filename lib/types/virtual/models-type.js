...
const INJECT_TYPE_METHODS = {
  ...
  'set': async function({ field, type }, models, options) {
    return this.getConnection(options && options.connection).transaction(async (connection) => {
      ...
      let storedModels    = await this[addToMethodName](models, Object.assign({}, options || {}, { connection }));
  ...
  'removeFrom': async function({ field, type }, _models, _options) {
    return this.getConnection(_options && _options.connection).transaction(async (connection) => {
      ...
      if (needsLoad()) {
        ...
        storedModels      = await this[getMethodName].call(this, query, Object.assign({}, options, { stream: false, includeRelations: true, connection }));
      } else {
        storedModels = models;
      }
  ...
  'has': async function({ field, type }, queryEngine, options) {
    ...
    let count           = await this[countMethodName].call(this, queryEngine, options);
...
class ModelsType extends RelationalTypeBase {
  ...
  initialize(connection, modelInstance) {
    ...
    if (!connection)
      throw new Error('ModelsType::initialize: A connection is required to use the ModelsType.');
