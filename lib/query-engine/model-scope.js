...
const ORDER_ASC   = Symbol.for('@mythix-orm/query-engine/model-scope/orderAsc');
const ORDER_DESC  = Symbol.for('@mythix-orm/query-engine/model-scope/orderDesc');

function wrapOrderClause(func) {
  func.DESC = (...args) => {
    return func.call(this, ORDER_DESC, ...args);
  };

  func.ASC = (...args) => {
    return func.call(this, ORDER_ASC, ...args);
  };
...
class ModelScope extends QueryEngineBase {
  ...
  margeFields(currentFields, incomingFields) {
    let context   = this.getOperationContext();
    let rootModel = context.rootModel;

    return QueryUtils.margeFields(rootModel, this.getConnection(), currentFields, incomingFields);
  }
  ...
  ORDER = wrapOrderClause.call(this, (...args) => {
    ...
    this._pushOperationOntoStack({ control: true, operator: 'ORDER', queryProp: 'ORDER', value: values, order: values });
    return this._fetchScope('model');
  });
  ...
  GROUP_BY(...args) {
    ...
    this._pushOperationOntoStack({ control: true, operator: 'GROUP_BY', queryProp: 'GROUP_BY', value: values, groupBy: values });
    return this._fetchScope('model');
  }
  ...
  PROJECT(...args) {
    ...
    this._pushOperationOntoStack({ control: true, operator: 'PROJECT', queryProp: 'PROJECT', value: values, projection: values });
    return this._fetchScope('model');
  }
