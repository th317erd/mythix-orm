...
class QueryEngineBase extends ProxyClass {
  ...
  _newFieldScope(Field) {
    const isBadOperation = () => {
      let queryParts    = this._getRawQuery();
      let previousPart  = queryParts[queryParts.length - 1];

      if (Object.prototype.hasOwnProperty.call(previousPart, 'condition') && previousPart.condition === true) {
        console.log(queryParts, queryParts.length);
        return true;
      }

      if (Object.prototype.hasOwnProperty.call(previousPart, 'operator') && previousPart.operator === 'FIELD') {
        console.log(queryParts, queryParts.length);
        return true;
      }
    ...
    if (isBadOperation())
      throw new Error(`QueryEngine::Field: Malformed query detected. No "AND" or "OR" condition proceeds the field "${fieldName}".`);
