...
class QueryEngineBase extends ProxyClass {
  ...
  filter(callback) {
    const Klass           = this.getQueryEngineScope().constructor;
    let context           = this._getRawQueryContext();
    let parts             = this._getRawQuery();
    let query             = new Klass({ ...context.rootContext, queryRoot: [] });
