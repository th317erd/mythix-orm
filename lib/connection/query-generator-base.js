...
class QueryGeneratorBase {
  ...
  getEscapedProjectionName(Model, field, options) {
    if (options && options.noProjectionAliases)
      ...
      return `${this.getEscapedColumnName(Model, field, options)} AS ${(options && options.alias) ? this.escapeID(options.alias) : this.getEscapedFieldName(Model, field, options)}`;
  }
  ...
  _getLiteralAlias(literal, options) {
    let alias = (literal.options && literal.options.alias) || (options && options.alias);
    if (Nife.isEmpty(alias))
      return '';

    return ` AS ${this.escapeID(alias)}`;
  }
