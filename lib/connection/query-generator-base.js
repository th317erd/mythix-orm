  ...
  getProjectedFields(queryEngine, _options, asMap) {
    ...
    for (let [ fullyQualifiedName, projectedScope ] of queryProjection) {
      ...
      if (Nife.instanceOf(value, 'string')) {
        ...
        if (typeof value.getFullyQualifiedFieldName === 'function')
          fullFieldName = value.getFullyQualifiedFieldName();
        else
          fullFieldName = this.parseFieldProjection(fullyQualifiedName);

        if (!fullFieldName)
          fullFieldName = fullyQualifiedName;

        if (fullFieldName)
          allProjectionFields.set(fullFieldName, result || fullyQualifiedName);
        else
          allProjectionFields.set(fullyQualifiedName, result || fullyQualifiedName);
