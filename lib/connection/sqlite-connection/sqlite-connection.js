...
class SQLiteConnection extends SQLConnectionBase {
  ...
  getDefaultFieldValue(type) {
    switch (type) {
      case 'AUTO_INCREMENT':
        return new SQLLiteral('AUTOINCREMENT', { noDefaultStatementOnCreateTable: true, remote: true });
      case 'DATETIME_NOW':
        return new SQLLiteral('(STRFTIME(\'%Y-%m-%d %H:%M:%f\', \'NOW\'))', { escape: false, remote: true });
      case 'DATE_NOW':
        return new SQLLiteral('(STRFTIME(\'%Y-%m-%d\', \'NOW\'))', { escape: false, remote: true });
