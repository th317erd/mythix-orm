  ...
  splitModelAndSubModels(Model, primaryModel, _relationMap) {
    ...
    const splitSubModels = (Model, model) => {
      ...
      Model.iterateFields(({ field, fieldName }) => {
        ...
        let TargetModel = fieldType.getTargetModel(this, { recursive: true, followForeignKeys: true });
        if (!TargetModel)
          return;
        ...
        if (fieldRelations) {
          ...
          for (let i = 0, il = modelNames.length; i < il; i++) {
            ...
            let modelInstance = (relationStatus.instance) ? relationStatus.instance : new RelatedModel(null, { connection: this });
            if (!(modelInstance instanceof RelatedModel))
              modelInstance = new RelatedModel(modelInstance, { connection: this });
