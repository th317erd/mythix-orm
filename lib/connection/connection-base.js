...
class ConnectionBase extends EventEmitter {
  ...
  async splitModelAndSubModels(Model, primaryModel, _relationMap) {
    ...
    const splitSubModels = async (Model, model) => {
      ...
      let promises = Model.iterateFields(async ({ field, fieldName }) => {
        ...
        if (!(value instanceof TargetModel)) {
          value = new TargetModel(value, { connection: this });
        ...
        let fieldRelations = await Utils.getRelationalModelStatusForField(this, value, field);
        if (fieldRelations) {
          ...
          for (let i = 0, il = modelNames.length; i < il; i++) {
            ...
            let self = (relationStatus.instance) ? relationStatus.instance : new RelatedModel(null, { connection: this });
            if (!(self instanceof RelatedModel))
              self = new RelatedModel(self, { connection: this });

            addModelInstance(relatedModelName, self);
          }
        }

        await splitSubModels(TargetModel, value, relationMap);
      });
    ...
    await splitSubModels(Model, primaryModel);

    return relationMap;
  }

  // eslint-disable-next-line no-unused-vars
  async prepareAllModelsAndSubModelsForOperation(Model, models, _options) {
    ...
    for (let i = 0, il = models.length; i < il; i++) {
      ...
      let subModels = await this.splitModelAndSubModels(Model, model);
  ...
  async bulkModelOperation(Model, _models, _options, beforeCallback, callback, afterCallback, afterOperationCallback) {
    ...
    let primaryModelName  = Model.getModelName();
    let groupedModelMap   = await this.prepareAllModelsAndSubModelsForOperation(Model, models, options);
    let alreadyStored     = {};
