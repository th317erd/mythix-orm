...
describe('DateTimeType', () => {
  ...
  it('can cast to type', () => {
    let value = Types.DATETIME.castToType({ value: '2001-01-01' });
    expect(value).toBeInstanceOf(Date);
    expect(value.toISOString()).toEqual('2001-01-01T00:00:00.000Z');

    value = Types.DATETIME.castToType({ value: undefined });
    expect(value).toBe(undefined);

    value = Types.DATETIME.castToType({ value: null });
    expect(value).toBe(null);
  });

  it('should throw an error on bad datetime string', () => {
    expect(() => Types.DATETIME.castToType({ value: 'derp' })).toThrow(new TypeError('DateTimeType::castToType: Value provided ("derp") can not be cast into a date.'));
  });
});
